...
class MPCParams(PythonMsg):
    ...
    Q: np.array = field(default=np.array((0, 0))) # quadratic state cost (will be set by caller)
...
class LMPC(MPC):
    ...
    def addTerminalComponents(self,x0):
        ...
        for jj in sortedLapTime[0:self.numSS_it]:
            # --- FIX: ensure integer number of points ---
            k_points = int(self.numSS_Points // self.numSS_it + 1)
            SS_PointSelected, uSS_PointSelected, Qfun_Selected = self.selectPoints(jj, self.zt, k_points)
    ...
    def selectPoints(self, it, zt, numPoints):
        """select (numPoints)-nearest neighbors to zt. These states will be used to construct the safe set and the value function approximation
        Arguments:
            x: current state
            u: current input
        """
        ...
        x = self.SS[it]
        u = self.uSS[it]
        numPoints = max(2, min(numPoints, x.shape[0]))
        ...
        # symmetric window when possible, else clamp to edges
        half = numPoints // 2
        if (MinNorm - half) >= 0 and (MinNorm + half) < x.shape[0]:
            indexSSandQfun = range(MinNorm - half, MinNorm + half + 1)
        else:
            start = max(0, MinNorm - half)
            stop  = min(x.shape[0], start + numPoints)
            indexSSandQfun = range(start, stop)

        SS_Points  = x[indexSSandQfun, :].T
        SSu_Points = u[indexSSandQfun, :].T
        Sel_Qfun = self.Qfun[it][indexSSandQfun]

        # --- robust empty-check for self.xPred (avoid array vs list comparison) ---
        if not hasattr(self, "xPred") or self.xPred is None:
            xpred_empty = True
        elif isinstance(self.xPred, np.ndarray):
            xpred_empty = (self.xPred.size == 0)
        else:
            xpred_empty = (len(self.xPred) == 0)
        ...
        if xpred_empty:
            ...
            sPred = self.xPred[:, 4]
            predCurrLap = self.N - np.sum(sPred > self.predictiveModel.map.TrackLength)
